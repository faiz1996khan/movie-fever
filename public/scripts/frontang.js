...
app.controller('myCntr',['$scope','$http','$location', '$anchorScroll','$window',function($scope,$http,$location,$anchorScroll,$window){
    $scope.getmovie = function(title){
        $http.get('https://api.themoviedb.org/3/search/movie?api_key={your_api_key}&query='+title+'&language=en-US&include_adult=true').then(function(list){
            $scope.all = list.data.results;
        })
    }
    $scope.goto = function(title){
        $http.get('https://api.themoviedb.org/3/search/movie?api_key={your_api_key}&query='+title).then(function(title){
    ...
    $scope.show_us = function(){
        ...
        $scope.movies_info = [];
            $http.get('https://api.themoviedb.org/3/movie/now_playing?api_key={your_api_key}&language=en-US&page=1').then(function(data){
...
app.controller('upCntr',['$scope','$http','$location', '$anchorScroll',function($scope,$http,$location,$anchorScroll){
    $scope.movies_info = [];
        $http.get('https://api.themoviedb.org/3/movie/upcoming?api_key={your_api_key}&language=en-US&page=1').then(function(data){
            let res = data.data.results;
            let chunk = 4;
            let local = [];
            for(let i=0 ; i<res.length ; i+=chunk){
...
app.controller('tv_detailCntr',['$scope','$http','$routeParams',function($scope,$http,$routeParams){
    $http.get('https://api.themoviedb.org/3/tv/'+$routeParams.id+'?api_key={your_api_key}&language=en-US').then(function(tv_details){
...
app.controller('tv_galleryCntr',['$scope','$http','$routeParams',function($scope,$http,$routeParams){
    $http.get('https://api.themoviedb.org/3/tv/'+$routeParams.id+'/videos?api_key={your_api_key}&language=en-US').then(function(vid_details){
...
app.controller('tv_detCntr',['$scope','$http','$routeParams',function($scope,$http,$routeParams){
    $http.get('https://api.themoviedb.org/3/tv/'+$routeParams.id+'/reviews?api_key={your_api_key}&language=en-US&page=1').then(function(review){
        $scope.tv_review = review.data.results;
    })
}])

app.controller('detailCntr',['$scope','$http','$routeParams',function($scope,$http,$routeParams){
    $http.get('https://api.themoviedb.org/3/movie/'+$routeParams.id+'?api_key={your_api_key}&language=en-US&adult=true').then(function(res){
...
app.controller('galleryCntr',['$scope','$http','$routeParams',function($scope,$http,$routeParams){
    $http.get('https://api.themoviedb.org/3/movie/'+$routeParams.id+'/videos?api_key={your_api_key}&language=en-US').then(function(videos){
        $scope.video = videos.data.results;
        $scope.url= $scope.video[0].key;
    })
    $http.get('https://api.themoviedb.org/3/movie/'+$routeParams.id+'/images?api_key={your_api_key}').then(function(images){
        $scope.img = images.data.backdrops;
    })
}])

app.controller('detCntr',['$scope','$http','$routeParams',function($scope,$http,$routeParams){
    $http.get('https://api.themoviedb.org/3/movie/'+$routeParams.id+'/reviews?api_key={your_api_key}').then(function(ans){
...
app.controller('similarCntr',['$scope','$http','$routeParams',function($scope,$http,$routeParams){
    ...
    $http.get('https://api.themoviedb.org/3/movie/'+$routeParams.id+'/similar?api_key={your_api_key}&language=en-US&page=1').then(function(mov){
...
app.controller('tv_similarCntr',['$scope','$http','$routeParams',function($scope,$http,$routeParams){
    ...
    $http.get('https://api.themoviedb.org/3/tv/'+$routeParams.id+'/similar?api_key={your_api_key}&language=en-US&page=1').then(function(mov){
        let movie = mov.data.results;
        let chunk = 4;
        let local = [];
        for(let i=0 ; i<movie.length ; i+=chunk){
...
app.controller('advanceCntr',['$scope','$http','$sortMovies',function($scope,$http,$sortMovies){
    ...
    $scope.adv_search = function(term,genres,year,sort_by){
        if(sort_by === 'Movie'){
        $http.get('https://api.themoviedb.org/3/search/movie?api_key={your_api_key}&query='+term+'&language=en-US&include_adult=true').then(function(list){
            let terms = list.data.results;
            $scope.ans_res = $sortMovies.Ans(terms,genres,year);
            $scope.url_path = 'details';
        })
    }else if(sort_by === 'TV'){
        $http.get('https://api.themoviedb.org/3/search/tv?api_key={your_api_key}&query='+term+'&language=en-US&page=1').then(function(shows){
